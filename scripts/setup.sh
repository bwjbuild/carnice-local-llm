...
detect_llama_binary() {
    # macOS with Homebrew
    if [[ "$OS" == "macos" ]] && [[ -f "/opt/homebrew/bin/llama-server" ]]; then
        LLAMA_SERVER="/opt/homebrew/bin/llama-server"
        LLAMA_QUANTIZE="/opt/homebrew/bin/llama-quantize"
    # Linux/Windows with source build
...
build_llama() {
    ...
    if [[ "$OS" == "macos" ]] && command -v brew &> /dev/null && ! brew list llama.cpp &> /dev/null; then
        echo_info "Installing llama.cpp via Homebrew..."
        brew install llama.cpp
        detect_llama_binary
        if [[ -n "$LLAMA_SERVER" ]]; then
            echo_success "llama.cpp installed via Homebrew"
            return
        fi
    fi
    ...
    detect_llama_binary
    if [[ -n "$LLAMA_SERVER" ]]; then
        echo_warn "llama-server found at $LLAMA_SERVER"
        echo_info "Skipping build (use Homebrew or rebuild manually if needed)"
        return
    fi

    # Fall back to source build
    if [[ "$OS" == "macos" ]]; then
        echo_info "Building llama.cpp from source with Metal..."
    else
        echo_info "Building llama.cpp from source with: ${BACKEND:-CPU-only}..."
    fi
...
download_model() {
    ...
    case $choice in
        1)
            ...
            echo_info "Download from: https://huggingface.co/kai-os/Carnice-9b-GGUF"
            echo_info "Place Carnice-9b-Q4_K_M.gguf in ~/llama-models/"
            echo ""
            echo_info "Alternative via huggingface-cli (if you have access):"
            echo "  huggingface-cli download kai-os/Carnice-9b-GGUF Carnice-9b-Q4_K_M.gguf \\"
...
download_base_and_quantize() {
    ...
    if [[ -z "$LLAMA_QUANTIZE" ]]; then
        echo_error "llama-quantize not found"
        exit 1
    fi
...
install_wrapper() {
    ...
    if [[ -z "$LLAMA_SERVER" ]]; then
        echo_error "llama-server not found. Cannot install wrapper."
        exit 1
    fi
